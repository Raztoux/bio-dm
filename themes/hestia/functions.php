...
function add_js_scripts() {
	wp_enqueue_script( 'script', get_template_directory_uri() . '/assets/js/scripts.js', array( 'jquery-hestia-material', 'jquery-ui-core' ),HESTIA_VERSION,true );
...
//Si clique au bouton on appelle partHit pour savoir qu'elle partie est visée
// if($_POST['calculate'] == 'Déterminer les lesions du patient') {
//   $return = json_encode(partHit());
//   echo $return;
// }
...
//Détermine la partie principale touché et retourne celle-ci, avec la partie
//secondaire et éventuellement le trauma lié
function partHit() {
  $rand = rand(0,100);

  //Crane et Cerveau
  if($rand >= 0 && $rand < 11) {
    $partieTouche = "Crâne et Cerveau";
    $partieSecToucheEtLesion = affectedSideCrane();
  }
  //Cou
  elseif($rand >= 11 && $rand < 21) {
    $partieTouche = "Cou";
    $partieSecToucheEtLesion = affectedSideCou();
  }
  //Thorax
  elseif($rand >= 21 && $rand < 31) {
    $partieTouche = "Thorax";
    $partieSecToucheEtLesion = affectedSideThorax();
  }
  //Abdomen
  elseif($rand >= 31 && $rand < 41) {
    $partieTouche = "Abdomen";
    $partieSecToucheEtLesion = affectedSideAbdomen();
  }
  //Colonne Vertébrale
  elseif($rand >= 41 && $rand < 51) {
    $partieTouche = "Colonne Vertébrale";
    $partieSecToucheEtLesion = affectedSideColonne();
  }
  //Membre supérieur
  elseif($rand >= 51 && $rand < 61) {
    $partieTouche = "Membre supérieur";
    $partieSecToucheEtLesion = affectedSideMembreSup();
  }
  //Membre inférieur
  elseif($rand >= 61 && $rand < 71) {
    $partieTouche = "Membre inférieur";
    $partieSecToucheEtLesion = affectedSideMembreInf();
  }
  //Peau
  elseif($rand >= 71 && $rand < 81) {
    $partieTouche = "Peau";
    $partieSecToucheEtLesion = affectedSidePeau();
  }
  //Rien
  else {
    $partieTouche = 'Aucune';
    $partieSecToucheEtLesion = array('PartieSec' => 'Aucune','Lesion' => 'Aucune');
  }

  //Retourne la partie principale touché et un tableau des parties secondaires
  //et lésions
  return(array('PartiePrincipale' => $partieTouche, 'PartieSecEtLesion' => $partieSecToucheEtLesion));
}
