jQuery(document).ready(function($) {
    ...
    $.ajax({
      type: "POST",
      url: ajaxurl,
      data: { calculate : clickBtnValue }
    }).done(function(msg) {
			console.log(msg);
      var obj = jQuery.parseJSON(msg);
      $('#json').html("<hr /> \
      <h1>Blessure du patient : </h1> \
      <p><span id='title'>Partie Globale touché</span> : <span id='info'>" + obj.PartiePrincipale + "</span></p> \
      <p><span id='title'>Partie impactée</span> : <span id='info'>" + obj.PartieSecEtLesion.PartieSec + "</span></p> \
      <p><span id='title'>Lésion engendrée</span> : <span id='info'>" + obj.PartieSecEtLesion.Lesion + "</span></p> \
      <hr />");
    });
	});
